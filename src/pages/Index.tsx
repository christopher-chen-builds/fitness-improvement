...
/* ─── History Tab (CRM-style workout log) ─── */
function HistoryTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {workouts.map((w: any) => {
            const hasProgression = w.exercise_logs?.some((e: any) => e.weight_change_flag);
            return (
              <div
                key={w.id}
                className="bg-card rounded-xl p-4 space-y-2 cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all"
      ...
      <Dialog open={!!selectedWorkout} onOpenChange={() => setSelectedWorkout(null)}>
        <DialogContent className="bg-card border-border max-w-sm mx-auto max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            {selectedWorkout?.exercise_logs?.map((ex: any) => (
